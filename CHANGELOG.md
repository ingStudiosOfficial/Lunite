...
*   **1.9.9** - New Lunite Bytecode Virtual Machine (LBVM) `core/lbvm.py`, ability to compile Lunite files into bytecode (`.lunac`) and run them, minor fixes, REPL updates (like multiline, etc), sandbox mode (`sandbox` command), debug mode (`debug` command), STD LIB expansion (`Crypto`).